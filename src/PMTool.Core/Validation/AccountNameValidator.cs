...
public static class AccountNameValidator
{
    ...
    public static string NormalizeAndValidate(string accountName)
    {
        ...
        if (name.Length == 0)
        {
            throw new ArgumentException("账号名不能为空。", nameof(accountName));
        }

        if (name is "." or "..")
        {
            throw new ArgumentException("账号名不能使用 “.” 或 “..”。", nameof(accountName));
        }

        if (name.Contains('\\', StringComparison.Ordinal) || name.Contains('/', StringComparison.Ordinal))
        {
            throw new ArgumentException("账号名不能包含路径分隔符。", nameof(accountName));
        }

        var invalid = Path.GetInvalidFileNameChars();
        if (name.IndexOfAny(invalid) >= 0)
        {
            throw new ArgumentException("账号名包含非法文件名字符。", nameof(accountName));
        }
